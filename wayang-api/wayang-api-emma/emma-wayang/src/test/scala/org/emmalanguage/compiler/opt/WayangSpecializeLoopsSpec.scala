...
class WayangSpecializeLoopsSpec extends BaseCompilerSpec with WayangAware {

  override val compiler = new RuntimeCompiler with WayangCompiler
  ...
  lazy val testPipeline: u.Expr[Any] => u.Tree =
    pipeline(true)(
      Core.lift,
      WayangSpecializeLoops.specializeLoops.timed
    ).compose(_.tree)
  ...
  protected override def wrapInClass(tree: u.Tree): u.Tree = {
    ...
    q"""
    class $Cls {
      def $run(..$prs)(implicit wayang: ${WayangAPI.WayangContext}) = $tree
    }
    """
  }
  ...
  "desugared for-loop #1" in withDefaultWayangEnv(implicit flink => {
    val act = testPipeline(reify {
      val N = this.N
      val edges = this.edges
      val paths = DataQuantaDataBag(edges)
    ...
    val exp = dscfPipeline(reify {
      ...
      val reslt = WayangNtv.repeat(paths)(N, paths => {
        val delta = comprehension[Edge[Int], DataBag]({
          val e1 = generator[Edge[Int], DataBag]({
            paths
          })
  ...
  "while loop #1" in withDefaultWayangEnv(implicit flink => {
    val act = testPipeline(reify {
      val N = this.N
      val edges = this.edges
      val paths = DataQuantaDataBag(edges)
  ...
  "while loop #2" in withDefaultWayangEnv(implicit flink => {
    val act = testPipeline(reify {
      val N = this.N
      val edges = this.edges
      val paths = DataQuantaDataBag(edges)
