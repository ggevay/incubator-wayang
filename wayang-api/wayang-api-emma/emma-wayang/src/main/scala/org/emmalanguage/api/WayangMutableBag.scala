...
class WayangMutableBag[K: Meta, V: Meta] private
(
...
//    for (s <- ss if s.changed) yield s.k -> s.v
  ...
  def copy(): MutableBag[K, V] =
    new WayangMutableBag[K, V](ss)
}

object WayangMutableBag {
  ...
  def apply[K: Meta, V: Meta](
    ...
    implicit wayang: WayangContext
  ): MutableBag[K, V] = ??? //new WayangMutableBag(FlinkOps.cache(for {
//    (k, v) <- init
//  } yield State(k, v)))
