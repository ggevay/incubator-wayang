...
class WayangMacro(val c: blackbox.Context) extends MacroCompiler with WayangCompiler {

  def onWayangImpl1[T](e: c.Expr[T]): c.Expr[T] =
    onWayang(loadConfig(configPaths()))(e)

  def onWayangImpl2[T](config: c.Expr[String])(e: c.Expr[T]): c.Expr[T] =
    onWayang(loadConfig(configPaths(Some(config.tree))))(e)

  def onWayang[T](cfg: Config)(e: c.Expr[T]): c.Expr[T] = {
