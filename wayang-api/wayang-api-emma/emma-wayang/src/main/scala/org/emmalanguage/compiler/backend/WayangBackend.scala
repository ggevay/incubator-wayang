  ...
  object WayangBackend {
    ...
    val transform = TreeTransform("WayangBackend.transform", tree => {
      ...
      val broadcastLambdas = (for {
        ...
        val tpe = api.Type.fun(Seq(ExecutionContext), f.info.widen.dealias)
        val nme = api.TermName.fresh(f)
        val lhs = api.TermSym(f.owner, nme, tpe)

        val ctx = api.TermSym.free(api.TermName.fresh("ctx"), ExecutionContext)
        ...
        val zs = core.ValDef(ys.head, rhs match {
          ...
          case _ => abort(
            s"""
              ...
              |Currently, broadcast variables in the Wayang backend are supported only in
